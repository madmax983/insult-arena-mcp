...
impl Duel {
    ...
    /// Returns a view of the current state of the duel.
    #[must_use]
    pub fn view(&self) -> DuelStateView {
        let (phase, next_to_act, winner) = match self.state {
        ...
        DuelStateView {
            phase,
            next_to_act,
            pending_insult: self.pending_insult.clone(),
